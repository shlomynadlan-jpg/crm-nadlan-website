...
export default function ContactForm({ propertyId, propertyCity, propertyType, agentPhone, agentName }: Props) {
  ...
  const tgHref = `https://t.me/+${waPhone}`
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6">
      ...
      {status === 'done' ? (
        <div className="text-center py-8">
          ...
          <p className="font-semibold text-slate-800">{t('successTitle')}</p>
          <p className="text-slate-500 text-sm mt-1">{t('successSubtitle')}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <label htmlFor={`${formId}-name`} className="sr-only">{t('nameLabel')}</label>
          <input
          ...
          />
          <label htmlFor={`${formId}-phone`} className="sr-only">{t('phoneLabel')}</label>
          <input
          ...
          />
          <label htmlFor={`${formId}-message`} className="sr-only">{t('messageLabel')}</label>
          <textarea
            id={`${formId}-message`} name="message"
            placeholder={t('messagePlaceholder')} value={message}
          ...
          <label className="flex items-start gap-2 text-xs text-slate-500 leading-relaxed cursor-pointer">
            ...
            <span>
              ...
              {t('privacyConsent').split('{link}')[1]} <span className="text-red-500">*</span>
            </span>
          </label>
          <label className="flex items-start gap-2 text-xs text-slate-500 leading-relaxed cursor-pointer">
            <input
          ...
          {status === 'error' && (
            <p role="alert" className="text-red-500 text-sm">{t('sendError')}</p>
          )}
