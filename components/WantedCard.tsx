...
function formatBudget(budget: number | null, dealType: string): string {
  if (!budget) return 'תקציב לפי פנייה'
  const formatted = `₪${budget.toLocaleString('he-IL')}`
  return dealType.includes('השכרה') ? `${formatted} / חודש` : formatted
}

export default function WantedCard({ r }: { r: PropertyRequest }) {
  const sizeText = r.size_min && r.size_max
    ? `${r.size_min}–${r.size_max} מ״ר`
    : r.size_min ? `מ-${r.size_min} מ״ר`
    : r.size_max ? `עד ${r.size_max} מ״ר`
    : null
    ...
    >
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-2">
        ...
        <span className="text-sm font-semibold" style={{ color: '#C9A84C' }}>
          💰 {formatBudget(r.budget, r.deal_type)}
        </span>
      </div>
